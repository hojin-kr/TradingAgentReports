...
from cointradingagents.dataflows.interface import get_top_coins
from cointradingagents.default_config import DEFAULT_CONFIG

COIN_REPORTS_DIR = Path(DEFAULT_CONFIG["coin_reports_dir"]).resolve()
COIN_EVAL_DIR = Path(DEFAULT_CONFIG["coin_eval_results_dir"]).resolve()
...
def normalize_symbol(symbol: str, quote: str = "USDT") -> str:
    ...
    if not clean.endswith(quote):
        clean = f"{clean}{quote}"
    return clean


def resolve_coin_list(requested: Optional[str], top_count: int) -> List[str]:
    if requested:
        entries = [
        ...
        return list(dict.fromkeys(entries))  # preserve order, remove duplicates

    top = get_top_coins(limit=top_count)
    if not top:
        raise RuntimeError("Unable to retrieve top coins from Binance.")
    return [entry["symbol"] for entry in top]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Markdown coin reports using CoinTradingAgents outputs."
    )
    parser.add_argument(
        "--ticker",
        help="Comma-separated coin tickers (e.g., BTC,ETH or BTCUSDT). "
        "Defaults to the top market-cap coins when omitted.",
    )
    parser.add_argument(
        "--date",
        default=datetime.now(timezone.utc).strftime("%Y-%m-%d"),
        help="Analysis date in YYYY-MM-DD (UTC). Default: today.",
    )
    parser.add_argument(
        "--locales",
        default="EN",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of top coins (by 24h Binance USDT quote volume) to run when no ticker is provided.",
    )
    parser.add_argument(
        "--from-log",
        action="store_true",
        help="Load from coin_eval_results state log instead of re-running the graph",
    )
    parser.add_argument(
        "--translate",
        action="store_true",
    ...
    tickers = resolve_coin_list(args.ticker, args.top_count)
    need_translation = args.translate and any(
        l.upper() not in ["EN", "EN-US", "EN-GB"] for l in locales
    )

    ensure_dir(COIN_REPORTS_DIR)
    ensure_dir(COIN_EVAL_DIR)
    ...
    for ticker in tickers:
        ...
        print(f"Generating reports for {ticker} on {trade_date}...")

        if args.from_log:
            final_state = load_state_from_log(ticker, trade_date)
        else:
            if graph is None:
                from cointradingagents.graph.coin_trading_graph import (
                    CoinTradingAgentsGraph,
                )

                config = DEFAULT_CONFIG.copy()
                graph = CoinTradingAgentsGraph(debug=False, config=config)
            final_state, _ = graph.propagate(ticker, trade_date)
            if need_translation:
                translator = graph.quick_thinking_llm

        if args.from_log and need_translation and translator is None:
            from cointradingagents.graph.coin_trading_graph import CoinTradingAgentsGraph
